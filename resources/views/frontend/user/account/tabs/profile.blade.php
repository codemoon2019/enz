<div class="table-responsive">
    <table class="table table-striped table-hover table-bordered">
        <tr>
            ...
            <td><img src="{{ $logged_in_user->picture('small') }}" class="user-profile-image"/></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>{{ $logged_in_user->created_at->timezone(get_user_timezone())->format(config('core.setting.formats.datetime_12')) }}
                ({{ $logged_in_user->created_at->diffForHumans() }})
            </td>
        </tr>
        <tr>
            <th>{{ __('labels.frontend.user.profile.last_updated') }}</th>
            <td>{{ $logged_in_user->updated_at->timezone(get_user_timezone())->format(config('core.setting.formats.datetime_12')) }}
