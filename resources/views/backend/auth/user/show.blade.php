                        ...
                        <li class="nav-item">
                            <a class="nav-link active" data-toggle="tab" href="#overview" role="tab"
                               aria-controls="overview" aria-expanded="true"><i
                                        class="fa fa-user"></i> {{ __('labels.backend.access.users.tabs.titles.overview') }}
                            </a>
                        </li>
                    </ul>
                ...
                <div class="col">
                    <small class="float-right text-muted">
                        <strong>{{ __('labels.backend.access.users.tabs.content.overview.created_at') }}
                            :</strong> {{ $user->created_at->timezone(get_user_timezone())->format(config('core.setting.formats.datetime_12')) }}
                        ({{ $user->created_at->diffForHumans() }}),
                        <strong>{{ __('labels.backend.access.users.tabs.content.overview.last_updated') }}
                            :</strong> {{ $user->updated_at->timezone(get_user_timezone())->format(config('core.setting.formats.datetime_12')) }}
                        ({{ $user->updated_at->diffForHumans() }})
                        @if ($user->trashed())
                            <strong>{{ __('labels.backend.access.users.tabs.content.overview.deleted_at') }}
                                :</strong> {{ $user->deleted_at->timezone(get_user_timezone())->format(config('core.setting.formats.datetime_12')) }}
