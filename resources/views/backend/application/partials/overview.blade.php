...
@section ('title', $model->subject .' - Course Inquiry Management | ' . app_name())
@section('content')
    @component('backend.includes.widgets.box-model', ['model' => $model])
        @slot('title')
            {{ $model->subject }}
        @endslot
        @slot('secondary_title', 'Application Inquiry Management')
        @slot('content')
        	@php
        		$details = [];
        		$details['type'] = 1; 
        	@endphp
            {!! markdown('frontend.mail.application.application_mail', ['model' => $model, 'details' => $details]) !!}
        @endslot
        @slot('footer')
            <div class="row">
                <div class="col">
