    ...
    */
    ...
    Route::group(['middleware' => 'guest'], function () {
        ...
        Route::post('login', 'LoginController@login')
            ->middleware(['throttle:' . config('auth.throttle.maxAttempts') . ',' . config('auth.throttle.retryAfter')])
            ->name('login.post');

        Route::group([
            'middleware' => 'site:main',
        ], function () {
            Route::get(config('access.admin_login_url'), 'LoginController@showAdminLoginForm')->name('admin.login');
            Route::post(config('access.admin_login_url'), 'LoginController@login')
                ->middleware(['throttle:' . config('auth.throttle.maxAttempts') . ',' . config('auth.throttle.retryAfter')])
        ...
        Route::get('account/confirm/resend/{uuid}',
